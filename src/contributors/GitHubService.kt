...
interface GitHubServiceSuspend {
    @GET("orgs/{org}/repos?per_page=100")
...
@OptIn(ExperimentalSerializationApi::class)
fun createGitHubServiceSuspend(username: String, password: String): GitHubServiceSuspend {
    val authToken = "Basic " + Base64.getEncoder().encode("$username:$password".toByteArray()).toString(Charsets.UTF_8)
    val httpClient = OkHttpClient.Builder()
        .addInterceptor { chain ->
            val original = chain.request()
    ...
    val retrofit = Retrofit.Builder()
        ...
        .client(httpClient)
        .build()
    return retrofit.create(GitHubServiceSuspend::class.java)
}
