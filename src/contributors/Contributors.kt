...
interface Contributors: CoroutineScope {
    ...
    fun loadContributors() {
        ...
        val serviceSuspend = createGitHubServiceSuspend(req.username, req.password)
        ...
        when (getSelectedVariant()) {
            ...
            SUSPEND -> { // Using coroutines
                launch {
                    val users = loadContributorsSuspend(serviceSuspend, req)
                    updateResults(users, startTime)
                }.setUpCancellation()
            }
            CONCURRENT -> { // Performing requests concurrently
                launch(Dispatchers.Default) {
                    val users = loadContributorsConcurrent(serviceSuspend, req)
            ...
            NOT_CANCELLABLE -> { // Performing requests in a non-cancellable way
                launch {
                    val users = loadContributorsNotCancellable(serviceSuspend, req)
                    updateResults(users, startTime)
                }.setUpCancellation()
            }
            PROGRESS -> { // Showing progress
                launch(Dispatchers.Default) {
                    loadContributorsProgress(serviceSuspend, req) { users, completed ->
                        withContext(Dispatchers.Main) {
            ...
            CHANNELS -> {  // Performing requests concurrently and showing progress
                launch(Dispatchers.Default) {
                    loadContributorsChannels(serviceSuspend, req) { users, completed ->
                        withContext(Dispatchers.Main) {
                            updateResults(users, startTime, completed)
                        }
                    }
